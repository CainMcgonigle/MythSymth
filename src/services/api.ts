...
import { Node, CreateNodeRequest, UpdateNodeRequest, ApiResponse, Edge, MapData } from '../types'
...
class ApiService {
    ...
    this.initializationPromise = (async () => {
      ...
      try {
        if (window.electronAPI) {
          ...
          finalBaseURL = backendUrl.endsWith('/') ? backendUrl.slice(0, -1) : backendUrl
        }
      } catch (error) {
        console.warn('Failed to get backend URL from Electron, using default:', error)
      } finally {
        this.baseURL = finalBaseURL
        this.axiosInstance = axios.create({ baseURL: this.baseURL, timeout: 5000 })
  ...
  async getNodes(type?: string): Promise<Node[]> {
    ...
    const response = await this.axiosInstance!.get<ApiResponse<Node>>(`/nodes`, { params })
  ...
  async updateNode(id: number, updates: UpdateNodeRequest): Promise<Node> {
    await this.ensureInitialized()
    const response = await this.axiosInstance!.put<Node>(`/nodes/${id}`, updates)
    return response.data
  }

  async updateNodePositions(positions: Array<{ id: number; x: number; y: number }>): Promise<void> {
    await this.ensureInitialized()
