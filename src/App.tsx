...
import { useNodes, useCreateNode, useUpdateNode, useDeleteNode, useApiHealth } from './hooks/useNodes'
import './App.css'
...
  // Removed editPanelVisible state, controlled by selectedNode now
...
  // Filter nodes in the component instead of in the query
  const filteredNodes = filter === 'all' ? allNodes : allNodes.filter(node => node.data.type === filter)
...
    try {
      await deleteNodeMutation.mutateAsync(Number(nodeId))
      
      // Remove from graph
      if (graphRef.current) {
...
    setFilter(newFilter)
    // No need to refetch - we're filtering client-side now
  }
